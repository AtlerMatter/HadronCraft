...
public class BlockGeneratorCombustion extends BlockContainer{
	...
	public BlockGeneratorCombustion(boolean isActive) {
		super(Material.iron);
	...
	public void registerBlockIcons(IIconRegister iconRegister){
		this.iconSides = iconRegister.registerIcon("hadroncraft:Machine_Side");
		this.iconTop = iconRegister.registerIcon("hadroncraft:Machine");
		this.iconBack = iconRegister.registerIcon("hadroncraft:Machine_Output");
		this.iconFront = iconRegister.registerIcon("hadroncraft:CombustionGenerator");
	}
	...
	public IIcon getIcon(int side, int metadata){
		...
		return this.iconSides;
	}
	
	@Override
	public Item getItemDropped(int par1, Random random, int par3){
		return Item.getItemFromBlock(HcBlocks.generatorCombustionIdle);
	}
	
	@Override
	public void breakBlock(World world, int x, int y, int z, Block oldblock, int oldmetadata) {
		if (!keepInventory){
			TileGeneratorCombustion tileentity = (TileGeneratorCombustion) world.getTileEntity(x, y, z);
	...
	public static void updateBlockState(boolean active, World worldObj, int xCoord, int yCoord, int zCoord){
		...
		if(active)
		{
			worldObj.setBlock(xCoord, yCoord, zCoord, HcBlocks.generatorCombustionActive);
		}
		else
		{
			worldObj.setBlock(xCoord, yCoord, zCoord, HcBlocks.generatorCombustionIdle);
		}
	...
	public boolean onBlockActivated(World world, int x, int y, int z, EntityPlayer player, int side, float hitX, float hitY, float hitZ){
		if(world.isRemote){
			...
			player.openGui(MainRegistry.modInstance, MainRegistry.guiIdGeneratorCombustion, world, x, y, z);
		}
		return true;
	}
	...
	public void onBlockPlacedBy(World world, int x, int y, int z, EntityLivingBase entity, ItemStack itemstack){
		...
		if(itemstack.hasDisplayName()){
			((TileGeneratorCombustion) world.getTileEntity(x, y, z)).setGuiDisplayName(itemstack.getDisplayName());
		}
	}
